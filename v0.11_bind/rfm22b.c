...
unsigned char hop_list[HOP_CHANNELS_COUNT];// = {HOP_CHANNELS}; 
unsigned char hopping_channel = 1;



SIGNAL(INT0_vect) 
{
	 if (RF_Mode == Transmit) RF_Mode = Transmitted; 
	 if (RF_Mode == Receive) RF_Mode = Received; 
}
...
*/
	...
	_spi_write(0x34, 64);    // 7 default value or   // 64 nibble = 32byte preamble 
	_spi_write(0x35, 0x20);
	
	 // synchronize word
	_spi_write(0x36, 0x2d);    
	...
	_spi_write(0x3e, DATA_PACKAGE_SIZE);    // total tx DATA_PACKAGE_SIZE byte 
	...
	_spi_write(0x6d, TX_POWER); // 7 set power max power 
	_spi_write(0x79, 0x00);    // no hopping 
	_spi_write(0x7a, 0x04);    // 40khz step size (10khz x value) // no hopping 
...
{
    hopping_channel++;
    if (hopping_channel>(HOP_CHANNELS_COUNT-1)) hopping_channel = 0;
...
{
	int channel = hopping_channel;
	channel+=hops;
	while(1)
	{
		if (channel>(HOP_CHANNELS_COUNT-1)) channel = channel-(HOP_CHANNELS_COUNT);
		else break;
	}
	hopping_channel = channel;
	_spi_write(0x79, hop_list[hopping_channel]);
}
